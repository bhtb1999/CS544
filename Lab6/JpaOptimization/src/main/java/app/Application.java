...
//        retrieveSchoolsWithStudents();
    ...
    private void insertSchools() {
        for (int x = 1; x <= 3; x++) {
            ...
            insertStudents(school);
            System.out.println("Inserting school  " + x);
        }
    }

    private void insertStudents(School school) {
        for (int x = 0; x < 5000; x++) {
            Student student = new Student("John " + x, "Doe" + x, "@gmail.com" + x);
    ...
    private void retrieveSchools() {
        ...
        schools.stream().forEach(System.out::println);
    ...
    private void retrieveSchoolsWithStudents() {
        ...
        List<School> schools = schoolRepository.getSchoolWithStudents();
        schools.stream().forEach(System.out::println);
...
//        }
        customerRepository.bulkUpdateName("James Bond");
        long finish = System.currentTimeMillis();
        long timeElapsed = finish - start;
